...
for ntide in 'GAUC':
    ...
    for i in range(FIRST_CHUNK_SIZE):
        ...
        if free_E == None or energy < free_E:
            #Tell
            print(f"protein: {JOB_NAME}; First: ntide: {ntide}/GAUC; chunk: {i+1}/5000; energy: {energy}")
    ...
    for ntide in 'GAUC':
        ...
        for append in [True, False]:
            ...
            for k in range(CHUNK_SIZE):
                ...
                #Check if best
                if free_E == None or energy < free_E:
                    print(f"protein: {JOB_NAME}; N_NTIDE: {i+2}/{N_NTIDES};  ntide: {ntide}/GAUC; chunk: {k+1}/5000; energy: {energy}")
                    free_E = energy
                    position = complex.positions[:]
                #Remember energies
                energies.append(energy)
